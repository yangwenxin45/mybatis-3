...
public class CglibProxyFactory implements ProxyFactory {
  ...
  static Object crateProxy(Class<?> type, Callback callback, List<Class<?>> constructorArgTypes, List<Object> constructorArgs) {
    ...
    try {
      ...
      // 如果没有找到writeReplace方法，则设置代理类继承WriteReplaceInterface接口，该接口中有writeReplace方法
  ...
  private static class EnhancedResultObjectProxyImpl implements MethodInterceptor {
    ...
    // 能够触发全局懒加载的方法名为"equals"、"clone"、"hashCode"、"toString"。这四个方法名在Configuration中被初始化
        ...
        synchronized (lazyLoader) {
          ...
          if (WRITE_REPLACE_METHOD.equals(methodName)) {
            ...
            if (lazyLoader.size() > 0) {
              // 存在懒加载属性。因此返回的信息要更多，不仅仅是原对象，还有相关的懒加载的设置等信息，因此使用CglibSerialStateHolder进行一次封装
              return new CglibSerialStateHolder(original, lazyLoader.getProperties(), objectFactory, constructorArgTypes, constructorArgs);
            } else {
              // 没有未懒加载的属性了，直接返回原对象进行序列化操作
              return original;
            }
          } else {
            ...
            if (lazyLoader.size() > 0 && !FINALIZE_METHOD.equals(methodName)) {
              ...
              if (aggressive || lazyLoadTriggerMethods.contains(methodName)) {
                ...
                // 调用了属性写方法，则先清除该属性的懒加载设置，该属性不需要被懒加载了
